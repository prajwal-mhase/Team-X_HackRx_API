...
- 📄 Upload **insurance, legal, or policy documents** (PDF, DOCX, EML)  
- 🤖 Uses **Google Gemini 2.0 Flash** for natural language understanding  
- 🔍 **Semantic search & clause retrieval** via FAISS embeddings (optional)  
- 💬 Ask questions in plain English and get concise, contextual answers  
- ✅ Outputs:  
  - Decision: `approved`, `denied`, `needs_follow_up`, `partial`  
  - Supporting clauses and rationale  
  - Confidence score  
- 🧠 Handles scanned PDFs with **OCR** fallback (if `pytesseract` installed)  
...
3. **LLM Processing**
   ...
   * Parses questions → structured queries (`intent`, `entities`, `subqueries`)
